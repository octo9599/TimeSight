...
async function createTermin() {
	...
	for (const user of users) {
		await axios.post(`${API}/termin_user`, {
			termin_id: termin.data.id,
...
<template>
	<form class="termin-card" @submit.prevent="createTermin">
		...
		<div class="field">
			...
			<input type="text" v-model="bezeichnung" placeholder="Familientreffen" required />
		</div>

		<div class="field">
			<label>Beschreibung</label>
			<input type="text" v-model="beschreibung" placeholder="Treffen bei Oma" />
		</div>
		...
		<div class="actions">
			<button type="submit" class="primary">Erstellen</button>
		</div>
	</form>
</template>
